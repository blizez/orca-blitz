...
export function ProvidersSettings() {
  ...
  const scrollToSelected = useCallback(() => {
    ...
    if (selected) {
      ...
      const offset =
        selectedRect.top - containerRect.top - containerRect.height / 2 + selectedRect.height / 2;
    ...
    try {
      ...
      if (activeProvider.id === "anthropic") {
        ...
        if (activeProvider.id === "ollama") {
          ...
          models =
            data.models?.map((m: { name: string }) => m.name.replace("models/", "")).sort() || [];
      ...
      if (msg.includes("Invalid API key")) {
        setDialog((prev) => ({
          ...prev,
          fetchError: "Invalid API key. Please check and try again.",
        }));
      } else if (msg.includes("Failed to fetch") || msg.includes("NetworkError")) {
        setDialog((prev) => ({
          ...prev,
          fetchError: "Cannot reach provider. Check your connection or API key.",
        }));
      } else {
  ...
  const openDialog = (providerId: string) => {
    ...
    if (providerId === "opencode") {
      ...
      if (existing?.authMethod === "chatgpt") {
        ...
        newDialog.authStatus = existing?.oauthToken
          ? "Connected! Type the model name you want to use."
          : null;
      } else {
  ...
  const startChatGPTAuth = async () => {
    if (!isElectron) {
      setDialog((prev) => ({
        ...prev,
        fetchError: "ChatGPT authentication requires the desktop app.",
      }));
      return;
    }
    ...
    const unsubModels = window.api.openai.onAuthModels(() => {
      setDialog((prev) => ({
        ...prev,
        authStatus: "Connected! Type the model name you want to use.",
        isAuthenticating: false,
      }));
      unsubUrl();
      unsubToken();
