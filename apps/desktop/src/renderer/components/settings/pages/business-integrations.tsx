...
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from '@orca-blitz/ui/components/ui/dialog'
...
import { ConnectWhatsAppCard } from '../../social-media/inbox/connect-whatsapp-card'
...
const channels: Array<{ id: AccountChannel; name: string; description: string }> = [
  ...
  { id: 'facebook', name: 'Facebook Messenger', description: 'Inicia sesión con tu email y contraseña.' },
...
export function BusinessIntegrations({
  ...
  useEffect(() => {
      ...
      const entries = await Promise.all(
        channels.map(async (item) => {
          ...
          if (item.id === 'gmail') {
            ...
            status = (await window.api.integrations.getStatus(businessId, item.id)) as ChannelSession
          }
    ...
    const removeStatus = window.api.integrations.onStatus((value) => {
      ...
      if (status.businessId === businessId) setStatuses((current) => ({ ...current, [status.channel]: status }))
    })
    const removeQr = window.api.integrations.onQR((value) => {
      if (value.businessId === businessId) {
        ...
        setStatuses((current) => ({
          ...current,
          whatsapp: { ...current.whatsapp, businessId, channel: 'whatsapp', status: 'qr' } as ChannelSession,
        }))
      }
    })
    return () => {
  ...
  async function connect(channel: AccountChannel): Promise<void> {
    ...
    toast.add({ title: `Conectando ${channel}...`, description: 'Se abrirá una ventana para iniciar sesión. Sigue los pasos del navegador.', type: 'info' })
    try {
      if (channel === 'gmail') {
      ...
      if (status.error === 'google_credentials_missing' || status.error === 'telegram_credentials_missing') {
        toast.add({ title: 'Servicio no disponible', description: 'La conexión con este canal no está configurada. Contacta al administrador.', type: 'error' })
      }
    } finally {
      setLoading(null)
    }
  ...
  async function submitLogin(): Promise<void> {
    ...
    setLoading(channel)
    setStatuses((current) => ({ ...current, [channel]: { businessId, channel, status: 'connecting' } as ChannelSession }))
    try {
      let status: ChannelSession
      if (channel === 'instagram') {
        status = (await window.api.integrations.instagramLogin(businessId, loginUser.trim(), loginPass)) as ChannelSession
      } else {
        status = (await window.api.integrations.messengerLogin(businessId, loginUser.trim(), loginPass)) as ChannelSession
      }
      setStatuses((current) => ({ ...current, [channel]: status }))
      ...
      setStatuses((current) => ({ ...current, [channel]: { businessId, channel, status: 'error', error: msg } as ChannelSession }))
    } finally {
      setLoading(null)
    }
  }
  ...
  async function disconnect(channel: AccountChannel): Promise<void> {
    ...
    setStatuses((current) => ({ ...current, [channel]: { businessId, channel, status: 'disconnected' } }))
  }
    ...
    <div className="space-y-6">
      {!embedded && (
        <header>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Conecta las cuentas de este negocio. Social Media usará estas conexiones para centralizar los mensajes.
          </p>
        </header>
      )}
        ...
        <DialogContent className="max-w-[420px] gap-0 border-0 bg-transparent p-0 shadow-none [&>button]:text-white [&>button]:hover:text-white">
          ...
          {flowChannel && (
            <ConnectWhatsAppCard
              ...
              qr={qr}
              onConnect={(value) => {
                if (flowChannel === 'telegram') void window.api.integrations.telegramStartLogin(businessId, value ?? '')
                else void window.api.integrations.connect(businessId)
              }}
              onSubmitCode={(value) => void window.api.integrations.telegramSubmitCode(businessId, value)}
              onSubmitPassword={(value) => void window.api.integrations.telegramSubmitPassword(businessId, value)}
              onDisconnect={() => {
                setQr(null)
                setFlowOpen(false)
                if (flowChannel === 'telegram') void window.api.integrations.telegramDisconnect(businessId)
      ...
      <Dialog open={!!loginChannel} onOpenChange={(o) => !o && setLoginChannel(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              Conectar {loginChannel === 'instagram' ? 'Instagram' : loginChannel === 'facebook' ? 'Facebook Messenger' : loginChannel}
            </DialogTitle>
            <DialogDescription>Inicia sesión con tu cuenta. Sin configuración extra.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <div className="space-y-1">
              <label className="text-sm font-medium">{loginChannel === 'instagram' ? 'Usuario' : 'Email'}</label>
              <Input
                value={loginUser}
            ...
            <p className="text-xs text-muted-foreground">Tus credenciales se guardan cifradas localmente.</p>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setLoginChannel(null)}>
              Cancelar
            </Button>
            <Button onClick={() => void submitLogin()} disabled={loading === loginChannel || !loginUser.trim() || !loginPass.trim()}>
      ...
      <div className="space-y-3">
        {channels.map((channel) => {
          ...
          return (
            <div key={channel.id} className="flex items-center gap-4 rounded-xl border border-border bg-card p-4">
      ...
      <div className="flex items-start gap-3 rounded-xl border border-border bg-muted/40 p-4 text-xs text-muted-foreground">
        ...
        <p>Las cuentas se guardan por negocio. No uses esta sección para envíos masivos: cada plataforma tiene sus propios límites y políticas.</p>
      </div>
    </div>
  )
}
