...
export function registerMetaAuthHandlers(): void {
  ...
  ipcMain.handle(
    "integrations:meta:getStatus",
    (_event, businessId: string, channel: MetaChannel) =>
      repository.getSession(businessId, channel),
  );
  ipcMain.handle(
    "integrations:meta:start",
    async (event, businessId: string, channel: MetaChannel) => {
      const sender = BrowserWindow.fromWebContents(event.sender);
      const config = getConfig();
      if (!config) {
        const session = saveStatus(businessId, channel, {
          status: "error",
      ...
      await new Promise<void>((resolve, reject) => {
        server = http.createServer(async (request, response) => {
          ...
          if (!code) {
            ...
            response.writeHead(400);
            response.end("Meta authorization failed");
            server?.close();
            server = null;
            resolve();
            return;
          ...
          try {
            ...
            const connected = saveStatus(businessId, channel, {
              status: "connected",
              error: undefined,
            });
            sender?.webContents.send("integrations:status", connected);
            response.writeHead(200, { "Content-Type": "text/html" });
            response.end("<h1>Meta conectado</h1><p>Puedes cerrar esta ventana.</p>");
          } catch (error) {
            const failed = saveStatus(businessId, channel, {
              status: "error",
              error: error instanceof Error ? error.message : "Meta token exchange failed",
            });
            sender?.webContents.send("integrations:status", failed);
...
function tokenPath(businessId: string, channel: MetaChannel): string {
  const directory = join(app.getPath("userData"), "messaging", businessId);
  mkdirSync(directory, { recursive: true });
  return join(directory, `${channel}.token`);
}
function saveToken(businessId: string, channel: MetaChannel, token: string): void {
  const value = safeStorage.isEncryptionAvailable()
    ? safeStorage.encryptString(token).toString("base64")
    : token;
  writeFileSync(tokenPath(businessId, channel), value, "utf8");
}
function saveStatus(
  businessId: string,
  channel: MetaChannel,
  update: Partial<ChannelSession>,
): ChannelSession {
...
export function readMetaToken(businessId: string, channel: MetaChannel): string | null {
  const path = tokenPath(businessId, channel);
