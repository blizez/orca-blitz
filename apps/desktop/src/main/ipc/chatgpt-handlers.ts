...
export function registerChatGPTHandlers(mainWindow: BrowserWindow) {
  ...
  ipcMain.handle('chatgpt:send', async (_event, model: string, messages: Array<{ role: 'user' | 'assistant'; content: string }>) => {
  ...
  ipcMain.handle('chatgpt:stream', async (_event, model: string, messages: Array<{ role: 'user' | 'assistant'; content: string }>) => {
    const token = tokens.get('current')
    if (!token) throw new Error('Not authenticated with ChatGPT')
