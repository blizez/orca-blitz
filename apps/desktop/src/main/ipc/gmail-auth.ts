...
export function registerGmailAuthHandlers(): void {
  ...
  ipcMain.handle('integrations:gmail:start', async (event, businessId: string) => {
    ...
    if (!config) {
      const session = saveStatus(businessId, { status: 'error', error: 'google_credentials_missing' })
...
function saveToken(businessId: string, token: string): void {
  const value = safeStorage.isEncryptionAvailable() ? safeStorage.encryptString(token).toString('base64') : token
...
export function readGmailToken(businessId: string): string | null {
  ...
  return safeStorage.isEncryptionAvailable() ? safeStorage.decryptString(Buffer.from(value, 'base64')) : value
}
