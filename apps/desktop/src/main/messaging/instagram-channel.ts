...
export class InstagramChannel {
      ...
      for (const thread of inbox.inbox.threads ?? []) {
        ...
        for (const item of (thread.items ?? []).slice(0, 5)) {
          ...
          const message: UnifiedMessage = {
            ...
            type: 'text',
            body: text,
            timestamp: Number((item as { timestamp?: string | number }).timestamp ?? Date.now() / 1000) * 1000,
            status: 'received',
          }
          this.persist(message)
        }
  ...
  private setStatus(update: Partial<ChannelSession>): void {
    this.session = { ...this.session, ...update, businessId: this.businessId, channel: 'instagram' }
