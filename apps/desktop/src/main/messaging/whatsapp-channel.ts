...
export class WhatsAppChannel {
  ...
  async disconnect(): Promise<void> {
    ...
    try { await this.socket?.logout() } catch { /* already disconnected */ }
  ...
  private async openSocket(): Promise<void> {
    ...
    socket.ev.on('connection.update', ({ connection, lastDisconnect, qr }) => {
      ...
      if (connection === 'close') {
        ...
        if (loggedOut || this.stopped) {
          ...
          this.reconnectTimer = setTimeout(() => { void this.openSocket() }, 2000)
        }
      }
    })
    socket.ev.on('contacts.upsert', (contacts) => {
      for (const contact of contacts) {
        if (contact.id) this.repository.upsertContact(this.businessId, contact.id, { name: contact.name ?? undefined, pushname: contact.notify ?? undefined, isGroup: contact.id.endsWith('@g.us') })
      }
    })
    socket.ev.on('messaging-history.set', ({ contacts, chats, messages }) => {
      ...
      for (const chat of chats) {
        if (chat.id) this.repository.upsertContact(this.businessId, chat.id, { name: chat.name ?? undefined, isGroup: chat.id.endsWith('@g.us') })
      }
      this.saveMessages(messages, false)
    })
    socket.ev.on('messages.upsert', ({ messages, type }) => this.saveMessages(messages, type === 'notify'))
  }

  private saveMessages(messages: import('baileys').proto.IWebMessageInfo[], countUnread: boolean): void {
    for (const rawMessage of messages) {
      const message = normalizeMessage(rawMessage, this.businessId)
      if (!message) continue
      this.repository.saveMessage(message)
      if (!countUnread && !message.fromMe) this.repository.markRead(message.businessId, message.chatJid)
