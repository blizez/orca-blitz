...
export class TelegramChannel {
  ...
  private sessionPath(): string {
    const directory = join(app.getPath("userData"), "messaging", this.businessId);
    mkdirSync(directory, { recursive: true });
    return join(directory, "telegram.session");
  }
  ...
  private writeSession(session: string): void {
    const value = safeStorage.isEncryptionAvailable()
      ? safeStorage.encryptString(session).toString("base64")
      : session;
    writeFileSync(this.sessionPath(), value, "utf8");
  }
  private setStatus(update: Partial<ChannelSession>): void {
    this.session = { ...this.session, ...update, businessId: this.businessId, channel: "telegram" };
