...
export class ChannelManager {
  ...
  status(businessId: string, channel: ChannelSession["channel"] = "whatsapp"): ChannelSession {
    if (channel === "telegram")
      return (
        this.telegramChannels.get(businessId)?.getStatus() ??
        this.repository.getSession(businessId, "telegram")
      );
    if (channel === "instagram")
      return (
        this.instagramChannels.get(businessId)?.getStatus() ??
        this.repository.getSession(businessId, "instagram")
      );
    if (channel === "facebook")
      return (
        this.messengerChannels.get(businessId)?.getStatus() ??
        this.repository.getSession(businessId, "facebook")
      );
  ...
  async sendText(
    ...
    jid: string,
    text: string,
    channel: ChannelSession["channel"] = "whatsapp",
  ): Promise<void> {
    if (channel === "telegram") {
