...
export class MessageRepository {
  ...
  saveMessage(message: UnifiedMessage): void {
    ...
    db.prepare(
      `UPDATE contacts SET last_message_at = ? WHERE business_id = ? AND jid = ? AND channel = ?`,
    ).run(message.timestamp, message.businessId, message.chatJid, message.channel);
    if (!message.fromMe) {
      db.prepare(
        `UPDATE contacts SET unread_count = unread_count + 1 WHERE business_id = ? AND jid = ? AND channel = ?`,
  ...
  setMediaPath(businessId: string, messageId: string, mediaPath: string): void {
    getDatabase()
      .prepare(`UPDATE messages SET media_path = ? WHERE business_id = ? AND id = ?`)
  ...
  markRead(businessId: string, jid: string, channel: Conversation["channel"] = "whatsapp"): void {
    getDatabase()
      .prepare(
        `UPDATE contacts SET unread_count = 0 WHERE business_id = ? AND jid = ? AND channel = ?`,
      )
    ...
    `)
      .run(
        session.channel === "whatsapp"
          ? session.businessId
          : `${session.businessId}:${session.channel}`,
        session.phone ?? null,
        session.name ?? null,
        session.status,
        Date.now(),
  ...
  getSession(businessId: string, channel: ChannelSession["channel"] = "whatsapp"): ChannelSession {
    ...
    const row = getDatabase().prepare(`SELECT * FROM sessions WHERE business_id = ?`).get(key) as
