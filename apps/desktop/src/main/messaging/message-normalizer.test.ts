...
describe("normalizeMessage", () => {
  ...
  it("descarta estados y newsletters", () => {
    expect(
      normalizeMessage(
        msg({ remoteJid: "status@broadcast", message: { conversation: "x" } }),
        "biz1",
      ),
    ).toBeNull();
    expect(
      normalizeMessage(
        msg({ remoteJid: "123@newsletter", message: { conversation: "x" } }),
        "biz1",
      ),
    ).toBeNull();
  });
  ...
  it("clasifica documento y usa el fileName como cuerpo", () => {
    const result = normalizeMessage(
      msg({
        remoteJid: "123@s.whatsapp.net",
        message: { documentMessage: { fileName: "cv.pdf" } },
      }),
      "biz1",
    );
  ...
  it("clasifica video, audio y sticker", () => {
    ...
    expect(normalizeMessage(msg({ ...base, message: { videoMessage: {} } }), "biz1")?.type).toBe(
      "video",
    );
    expect(normalizeMessage(msg({ ...base, message: { audioMessage: {} } }), "biz1")?.type).toBe(
      "audio",
    );
    expect(normalizeMessage(msg({ ...base, message: { stickerMessage: {} } }), "biz1")?.type).toBe(
      "sticker",
    );
  });
  ...
  it('retorna type "other" con body vacío para contenido desconocido', () => {
    const result = normalizeMessage(
      msg({ remoteJid: "123@s.whatsapp.net", message: { contactMessage: {} } }),
      "biz1",
    );
  ...
  it("usa Date.now() cuando no hay timestamp", () => {
    ...
    const result = normalizeMessage(
      msg({ remoteJid: "123@s.whatsapp.net", message: { conversation: "x" } }),
      "biz1",
    );
